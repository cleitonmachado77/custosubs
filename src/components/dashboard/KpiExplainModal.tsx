...
export function KpiExplainModal({ explain, onClose }: KpiExplainModalProps) {
  // Fecha com ESC
  useEffect(() => {
    if (!explain) return
    ...
    >
      ...
      >
        ...
        <div className="px-5 py-4 border-b border-gray-100">
          ...
          <p className="text-2xl font-extrabold text-[#01884d]">{explain.value}</p>
          {explain.formula && (
          ...
          {explain.steps.map((step, i) => (
            <div
              ...
              className={[
                ...
                step.highlight
                  ? 'bg-[#01884d]/8 border border-[#01884d]/20 font-semibold'
                  : 'bg-gray-50 border border-gray-100',
              ].join(' ')}
            >
              <span className={step.highlight ? 'text-[#01884d]' : 'text-gray-600'}>
                {step.label}
              </span>
              <span className={step.highlight ? 'text-[#01884d] font-bold' : 'text-gray-800 font-semibold'}>
                {step.value}
              </span>
            </div>
          ))}
        ...
        {explain.note && (
          <div className="px-5 pb-4">
            <p className="text-xs text-gray-400 bg-amber-50 border border-amber-100 rounded-xl px-3 py-2 leading-relaxed">
              💡 {explain.note}
            </p>
          </div>
        )}
